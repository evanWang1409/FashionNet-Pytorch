...
if __name__ == '__main__':
	...
	net = resnet101(pretrained = True, num_classes = len(classes))
	...
	loss_file = open('fas_resnet_men_101.csv', 'w')
	...
	for epoch in range(epochs):  # loop over the dataset multiple times
		...
		for i, data in enumerate(trainloader):
			...
			if count % 1000 == 999:	# print every 2000 mini-batches
				print('[%d, %d] loss: %.3f' % (epoch + 1, count + 1, running_loss / 1000))
				writer.writerow([count, running_loss/1000])
				running_loss = 0.0

			if count%10000 == 0:
				torch.save(net.state_dict(), 'fas_resnet101_men_{}.pt'.format(count))
	...
	women_losses = []
	loss_file = open('fas_resnet_women_101.csv', 'w')
	writer = csv.writer(loss_file, delimiter = ',')
	...
	for epoch in range(epochs):  # loop over the dataset multiple times
		...
		for i, data in enumerate(trainloader):
			...
			if count%10000 == 0:
				torch.save(net.state_dict(), 'fas_resnet101_women_{}.pt'.format(count))
				print('men_{}saved'.format(count))
